program SIMRSSettings;

{
  SmartOffice Desktop - Pengaturan module (standalone executable).
...
begin
  ...
  try
    ...
    DrawAppIcon(Bmp, RGBToColor(31, 78, 121), gSettings);
    Ico := TIcon.Create;
    try
      Ico.Assign(Bmp);
...
begin
  ...
  SetAppIcon;
  if not EnsureSingleInstance('Pengaturan') then
    Exit;
  InitApp;
  try
    ...
    ModuleHostForm.Setup('Pengaturan', TSettingsView.Create(nil));
    LogMsg('Modul Pengaturan dibuka oleh ' + RBAC.Username);
    Application.Run;
  except
    on E: Exception do
    begin
      LogMsg('Pengaturan: ' + E.Message);
