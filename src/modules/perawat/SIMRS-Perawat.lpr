program SIMRSPerawat;

{
  SmartOffice Desktop - Perawat module (standalone executable).
...
begin
  ...
  try
    ...
    DrawAppIcon(Bmp, RGBToColor(31, 78, 121), gNurse);
    Ico := TIcon.Create;
    try
      Ico.Assign(Bmp);
...
begin
  ...
  SetAppIcon;
  if not EnsureSingleInstance('Perawat') then
    Exit;
  InitApp;
  try
    ...
    ModuleHostForm.Setup('Perawat', TPerawatView.Create(nil));
    LogMsg('Modul Perawat dibuka oleh ' + RBAC.Username);
    Application.Run;
  except
    on E: Exception do
    begin
      LogMsg('Perawat: ' + E.Message);
